...
MRRG is a production-oriented roofing business management platform developed specifically for Margaret River Re-Gutter, a roofing company based in Western Australia.

The application digitizes the complete workflow of roofing jobs, from scheduling and employee assignment to field reporting, manager validation, notifications and job archiving.

While built for a real business, the project also serves as my portfolio, showcasing how I design and develop maintainable business software using Java, Spring Boot and React.
...
```
Docker Compose orchestrates three isolated containers:
- React frontend
- Spring Boot backend
- PostgreSQL database
...
1. Managers create roofing jobs.
2. Jobs are scheduled and assigned to employees.
3. Employees upload before photos directly from the field.
4. Employees complete the work and upload after photos.
5. Managers validate completed jobs.
6. Validated jobs are archived.
7. Customer callbacks automatically restore archived jobs to the scheduling queue with elevated priority.
