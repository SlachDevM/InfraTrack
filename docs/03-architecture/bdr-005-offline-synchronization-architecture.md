...
- **Idempotency (DT-OFFLINE-1 / RC-FIX-BE-1 delivered):** `operationId` is the client idempotency key. Processed operations are stored in `mobile_sync_operation` (outcome metadata only — no payload). The backend inserts a `PROCESSING` reservation before handler execution; only one concurrent request may execute a given `operationId`. Duplicates receive the recorded `SyncOperationResponse`. Runtime handler failures delete the reservation so retries are not permanently blocked. Records are retained for **90 days** (configurable) with scheduled cleanup.
