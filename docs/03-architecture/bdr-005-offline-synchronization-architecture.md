...
| **Work order progress (draft maintenance notes)** | Server replace on upload (**M6.1-BE1**) | While work order remains `ASSIGNED` and no `MaintenanceActivity` exists, assigned worker may save `completionNotes` draft to `work_orders.draft_completion_notes`. Does not complete the work order or create history. |
...
- **Idempotency (DT-OFFLINE-1 / RC-FIX-BE-1 delivered):** `operationId` is the client idempotency key. Clients must generate a new UUID per pending operation. Reusing an `operationId` with a different `entityType`, `operationType`, or `entityId` is rejected at operation level (`REJECTED`) without mutating the stored record. Processed operations are stored in `mobile_sync_operation` (outcome metadata only — no payload). The backend inserts a `PROCESSING` reservation before handler execution; only one concurrent request may execute a given `operationId` with the same signature. Duplicates with the same signature receive the recorded `SyncOperationResponse`. Runtime handler failures delete the reservation so retries are not permanently blocked. Records are retained for **90 days** (configurable) with scheduled cleanup.
- **Synchronization watermark (RC-FIX-BE-1):** A single server instant is captured after operation processing. Delta download filters `updatedAt <= watermark`; `nextSyncToken.issuedAt` uses the same watermark so no committed update silently falls behind the issued cursor.
- **Download correctness (M5.4 / M6.1-BE2):** prefer full delta over incorrect incremental sync. Invalid `syncToken` returns a single `FULL_SYNC_REQUIRED` warning and full inspection/work order deltas; sync handshake still succeeds.
...
- **Observability (M5.4.1 / DT-OFFLINE-1):** Micrometer metrics (`mobile.sync.requests`, `mobile.sync.operations.accepted|rejected|ignored|conflict|duplicate`, `mobile.sync.delta.inspections`, `mobile.sync.delta.work_orders`, `mobile.sync.duration`) and structured INFO logging per sync (user id and counts only — no JWT, payloads, or PII).
- **Sync scalability (V2.5-STAB-2 / M6.1-BE2):** Indexes on `inspections.assigned_to_user_id`, `inspections.updated_at`, and `work_orders.updated_at` (V34) support mobile list and incremental delta queries. Incremental delta uses repository-level `updatedAt` filtering with unchanged administrator/manager/field scoping. Inspection answers are loaded in one batch query per sync handshake.
...
M6.1 — Work Order Sync Upload (M6.1-BE1 delivered)
