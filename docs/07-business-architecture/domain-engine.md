...
**M5.4.2-BE (delivered):** Inspection delta checklist definitions — each scoped inspection delta embeds `template`, `questions[]`, and `choices[]` (when applicable) using the same mapping rules as `GET /api/mobile/inspections/{id}/bundle` via `MobileInspectionChecklistLoader`. Batch-loaded per sync (no N+1, no per-inspection bundle calls). `delta.inspections` is self-contained for offline rendering; Android does not need a prior online bundle fetch. Additive `choiceId` on answers; existing fields unchanged. No new endpoint; no workflow or authorization changes.
...
**M5.5-BE1 (delivered):** Conflict detection for `SAVE_INSPECTION_PROGRESS` only. Stale workflow, permission, or entity-state failures return `CONFLICT` plus a `conflicts[]` entry classified by `SyncConflictType` (`WORKFLOW_COMPLETED`, `ENTITY_DELETED`, `PERMISSION_DENIED`, `VERSION_MISMATCH`, `UNKNOWN`). Malformed payloads and answer validation failures remain `REJECTED`. No automatic resolution, tombstones, or document conflict handling. Android must retain conflicting operations for future UX. Work order progress conflicts added in **M6.4-BE1**.
...
**RC-FIX-BE-1 (delivered):** Release Candidate backend concurrency hardening — atomic idempotency (`PROCESSING` reservation via `saveAndFlush` before handler execution; concurrent duplicates receive recorded outcome; runtime failures release reservation). Synchronization watermark: single server instant after operation processing bounds delta (`updatedAt <= watermark`) and `nextSyncToken.issuedAt`. Inspection completion protected by JPA `@Version` optimistic locking (second concurrent completion returns existing business validation error; side effects once). `SyncResponse.requiresFullSync` reflects `FULL_SYNC_REQUIRED` warnings.
...
**M6.5-STAB-1 (delivered):** Expanded sync observability — per-section delta counters (`mobile.sync.delta.inspections|work_orders|assets|reference_data|dashboard`), per-section generation timers (`mobile.sync.delta.section.duration` tagged `section`), response size distribution (`mobile.sync.response.size.bytes`), and enriched structured logging (`deltaAssetCount`, `referenceDataIncluded`, `dashboardIncluded`). Performance review: inspections/work orders batch-loaded (no N+1); reference data loaded once per section; dashboard reuses `MobileService.buildDashboard`; `updatedAt` indexes on inspections (V30) and work orders (V34). Asset enrichment uses batch `findByIdIn` but per-asset context queries remain — documented caveat for future batching. No API, behaviour, or schema changes.
...
**M6.5-STAB-3-FIX (delivered):** Asset context sync hardening — `buildAssetContextsForSync` batches open issues (`NOT EXISTS` operational decision), active inspections/work orders, last inspection/maintenance, preventive plans, and asset-owned documents via `IN (...)` queries with in-memory grouping. Reporting exports enforce `reporting.export.max-rows` (default 50,000). Field-employee operational document listing uses SQL visibility filters with correct pagination totals. No API or workflow changes.

**M6.4-BE1 (delivered):** Work order progress conflict detection — `SAVE_WORK_ORDER_PROGRESS` on `WORK_ORDER` returns `CONFLICT` plus enriched `conflicts[]` when server state changed since the operation was queued (completed/cancelled/not `ASSIGNED`, maintenance activity exists, entity deleted, permission/assignment loss). Reuses `SyncConflictClassifier` and `SyncConflictEnrichment` (`serverState` with `status`, `updatedAt`, `assignedTo`, `assignedToName`; `clientState` with original payload). Malformed payload and `completionNotes` validation remain `REJECTED`. No automatic resolution; no offline completion (`COMPLETE_MAINTENANCE` deferred). Android must retain conflicting work order pending operations. Idempotency stores conflict outcome; duplicate `operationId` returns the same `CONFLICT` classification.
