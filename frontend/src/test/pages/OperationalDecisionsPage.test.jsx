...
describe('OperationalDecisionsPage', () => {
  ...
  it('shows eligible issue in selector from backend response', async () => {
    issueApi.listEligibleForOperationalDecision.mockResolvedValue(
      pageResponse([eligibleIssue])
    );
  ...
  it('shows empty selector message when no eligible issues exist', async () => {
    ...
    expect(await screen.findByText(/no issues are currently awaiting an operational decision/i)).toBeInTheDocument();
  ...
  it('records operational decision after selecting eligible issue', async () => {
    ...
    await waitFor(() => {
      expect(operationalDecisionApi.create).toHaveBeenCalledWith(expect.objectContaining({
        issueId: 10,
  ...
  it('displays forbidden message when backend rejects unauthorized decision', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('You do not have permission to make operational decisions.')).toBeInTheDocument();
    });
  });
